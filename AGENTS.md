...
**Not Working:**
- **Steam-generated haptics** — Trackpad clicks, UI feedback haptics, and other Steam-internal haptic events do NOT produce rumble. These come from Steam's own haptic system, not from `SDL_RumbleJoystick()`. The Steam haptic path uses a different code path that does not reach the Neptune motors. Only games that call `SDL_RumbleJoystick()` produce rumble. Root cause: 0x8F never dispatched on BLE because `[r15+0x208]` gate stays 0. The connection between `[rdi+0x1d8]` at the dispatcher and the YieldingRunTestProgram path is **UNVERIFIED** — 0x1d8 may be graphics API type, not controller state. See `research/steamclient-reverse-session/functions/yielding_run_test_program.c` for details.
...
1. **⚠️ CRITICAL: ALL PRIOR BINARY ANALYSIS WAS ON THE WRONG BINARY** — We analyzed `linux64/steamclient.so` (46MB, 64-bit), but Steam loads `ubuntu12_32/steamclient.so` (49MB, **32-bit**). All addresses, function offsets, and disassembly are WRONG. The YieldingRunTestProgram string exists in both binaries but at different offsets (32-bit: `0x00bfc7e3`, 64-bit: `0x00d6d17b`). **Must redo all RE on the 32-bit binary.** The GDB approach is now even more critical — it will work on the actual running process regardless of binary version.
2. **GDB watchpoint to verify 0x1d8 value (RECOMMENDED NEXT STEP)** — The 0x8F gate at `[r15+0x208]` is confirmed in the 64-bit binary, but we need to find the equivalent in the 32-bit binary first. Use GDB to catch the dispatcher when it runs and read [0x1d8]. The dispatcher is in the 32-bit binary, loaded at runtime.
3. **LD_PRELOAD patch for 0x8F gate (AFTER GDB VERIFICATION)** — Once the dispatcher path is verified, write a C library loaded via `LD_PRELOAD` that patches the conditional jump. 55-65% probability of working.
